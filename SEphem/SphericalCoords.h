...
namespace SEphem {
  ...
  class SphericalCoords
	...
	m_theta(theta), m_phi(phi) { rationalize(); }
      ...
      void rotatePhi(const Angle& phi) { m_phi.rotate(phi); }
      void rotatePhiRad(double phi) { m_phi.rotateRad(phi); }
      void rotatePhiDeg(double phi) { m_phi.rotateDeg(phi); }
      void rotatePhiRot(double phi) { m_phi.rotateRot(phi); }
  ...
  {
    m_phi.rotateRad(psi);
    double x=sin(m_theta)*cos(m_phi);
    double y=sin(m_theta)*sin(m_phi);
    double z=cos(m_theta);
