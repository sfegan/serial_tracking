...
doAzElCorrections(double& az_driveangle, double& el_driveangle,
		  const double& tel_az_driveangle, bool do_corrections,
		  DirectionPreference dp) const
{
  ...
  if((enable_offsets)&&(do_corrections)&&(enable_corrections))
    {
      ...
      el = el - (flex_el_A*cos(el)+flex_el_B*sin(2*el)); // zero at zenith
  ...
  bool cw_closest = fabs(az1-tel_az_driveangle)<=fabs(az2-tel_az_driveangle);

  if((az1>sc_lim_az_cw)&&(az2<sc_lim_az_cc))return false; // never happen
  else if(az1>sc_lim_az_cw)az=az2;
  else if(az2<sc_lim_az_cc)az=az1;
  else if((dp==DP_CW)||((dp==DP_NONE)&&(cw_closest)))az=az1;
  else az=az2;
